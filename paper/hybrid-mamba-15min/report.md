...
**Mixture-of-Experts**。Shazeer et al. [11] 提出 sparsely-gated MoE，允許模型容量以超線性方式擴張；Switch Transformer [13] 簡化 router 為 top-1 並導入 auxiliary load-balance loss；GShard [12] 解決跨設備分片；GLaM [14] 使用 sparsely activated MoE 達成推論 FLOPs 約為同級 dense 模型的一半；Mixtral 8×7B [15] 以 top-2 MoE 於工業規模達成 dense 相當的生成品質。然而所有這些系統的每個專家仍為完整的 FFN 矩陣，使權重總體積與跨設備通訊成本同步成長。ST-MoE [2] 引入 Router Z-loss 以穩定 Router logits，本研究沿用此設計並進一步以 fast scaled tanh 做硬截斷。近年 DeepSeek-V3 [5] 提出 auxiliary-loss-free load balancing，而 Qiu et al. [4] 則指出 load-balance loss 與 z-loss 會鼓勵 routing scores 趨於 uniform，與語言建模目標存在拉扯。

**張量分解用於權重壓縮**。Tucker 分解、CP 分解等技術長期用於壓縮卷積層與全連接層。TD-MoE [6] 首次將 Tucker 三階分解應用於 MoE 專家集合：將同一層所有 experts 的 weights 疊成 3D tensor，再做 joint Tucker decomposition，以 multi-linear whitening 與 3D rank allocation 捕捉跨 expert 冗餘。近年 Phi-3.5、Mixtral [15] 等 MoE 模型催生了 MoE-SVD、SVD-LLM 等逐專家壓縮方法，但實驗顯示這些方法在壓縮率超過 50% 時會大幅損失表達能力，核心原因在於忽略了專家之間的冗餘。本研究沿用 TD-MoE [6] 的 cross-expert tensorization 思路，透過共享 $U^{(2)}, U^{(3)}$ 同時達成**跨專家權重共用**與**單專家選擇性**，相關理論推導詳見 §5.4.1。
...
以下列出本實驗對應 `train.py` 中的 `Mamba3Config` 核心超參數設定：
...
General SFT 採用一般指令資料與多輪對話資料的混合版本。為避免實驗名稱與論文敘述脫節，本研究中各資料集的角色定義如下。

| 資料集名稱           | 階段                 | 樣本來源與用途                                                            | 格式約束                        |
| -------------------- | -------------------- | ------------------------------------------------------------------------- | ------------------------------- |
| `alpaca_hf`          | General SFT 原始來源 | Alpaca instruction/input/output 轉換為 user/assistant 對話                | ChatML                          |
| `ultrachat_200k/hf`  | General SFT 原始來源 | UltraChat 200k 對話快照，提供多輪互動與長回答                             | ChatML                          |
| `mix_a25_u75_hf`     | General SFT 混合版本 | Alpaca 與 UltraChat 依可用樣本混合，形成早期指令資料池                    | ChatML-only                     |
| `mix_a25_u75_cot_hf` | General SFT 主訓練集 | 第一階段實際使用版本；保留指令與多輪對話能力，但不注入 `<final>` 推理標籤 | 0 筆含 `<s>`、`</s>`、`<final>` |
| `sft_cot_hf`         | CoT-SFT 推理專化資料 | 每筆包含問題、推理內容與最終答案，用於強化步驟化推理與答案收束            | ChatML 外殼 + CoT 內部標記      |

第一階段 `mix_a25_u75_cot_hf` 的可用樣本總數為 **251,974** 筆，來源分布約為 **20.6%** 一般指令資料與 **79.4%** 大規模對話資料；經 tokenizer 後總量約 **302,597,070 tokens**。此分布使模型先學到足夠寬的任務覆蓋與自然對話流，再進入第二階段的推理專化，降低小型 CoT 資料直接覆蓋通用語言能力的風險。

本研究另對自定義 SFT/CoT 資料集執行 packing 前稽核，結果如下。該資料集完全符合本節設計：截斷率極低、有效 loss-token 比例高，且沒有低於 30% 有效學習比例的低效率樣本。

| 稽核項目                     | 數值                        | 論文詮釋                                                     |
| ---------------------------- | --------------------------- | ------------------------------------------------------------ | ------ | --- |
| 資料集總 token 量            | **248,435 tokens**          | 足以作為第二階段指令/推理專化資料，但不應取代 General SFT    |
| 樣本數                       | **1,032 筆**                | 屬於高密度客製化 SFT 集，適合 packing                        |
| 稽核序列長度 `seq_len`       | **1024**                    | 覆蓋絕大多數樣本長度，保留完整終止符學習訊號                 |
...
會被轉換為包含特殊 token、role token、換行 token 與 BPE 子詞的 input-id 序列。圖 SFT-1 展示其中一個對齊切片：`<|im_start|>` 對應 32000，`<|im_end|>` 對應 32001，`user`、`assistant`、問題與答案再依 BPE 詞表切成普通 token。這個視角能直接說明 SFT 的核心：模型不是在「看懂 Markdown」，而是在固定整數序列上學習何處該預測、何處只作為條件。

![ChatML SFT mask and input-id trace](assets/plots/sft_chatml_mask.png)

_圖 SFT-1：ChatML-only SFT 的 input-id、切詞與 loss mask 對齊示意。灰色區域代表 labels = `-100`，不直接計算交叉熵；綠色區域代表 assistant answer 與終止 token 參與 loss。HTML 原型位於 `prototypes/sft_chatml_mask.html`，可重現本圖的排版設計。_
...
![圖 C1：Causal Attention Mask 矩陣（8-token 示例，下三角結構）](./assets/plots/causal_mask.png)

![圖 C2：SFT Token 級別 Loss Mask 逐 token 可視化](./assets/plots/sft_loss_mask.png)

#### 7.2.6 Frobenius 誤差與矩陣分解品質
...
**與逐專家 SVD 的比較**。Tucker 嚴格保證定理（§5.4.3）確立了：以相同參數預算下，Tucker 聯合分解的 Frobenius 誤差**恆不大於**逐專家 SVD 的誤差，即
...
#### 7.2.7 SFT Mask 自動化驗證結果

下表彙整針對 3 個 SFT held-out 樣本執行自動化 mask 驗證的結果。全部樣本通過四項結構不變性檢驗，確認 ChatML-only loss mask 在多輪對話中正確施加。

_表 SFT-V：SFT Loss Mask 自動化驗證摘要（Sample #0–#2）。GPU 環境：NVIDIA GeForce RTX 3090（sm_86），bf16 + TF32 + FlashSDP。_

| 樣本 | 總 tokens | 受監督 tokens | 受監督比例 | 助理輪次 | Header 完全遮蔽 | 首答 token 受監督 | `<\|im_end\|>` 受監督 | 多輪覆蓋 |
| ---- | --------: | ------------: | ---------: | -------: | :-------------: | :---------------: | :-------------------: | :------: |
| #0   |     1,014 |           675 |     66.6 % |        6 |        ✓        |         ✓         |           ✓           |    ✓     |
| #1   |       849 |           699 |     82.3 % |        4 |        ✓        |         ✓         |           ✓           |    ✓     |
| #2   |       824 |           661 |     80.2 % |        3 |        ✓        |         ✓         |           ✓           |    ✓     |

四項不變性定義如下：（i）所有助理區塊的 header tokens（`<|im_start|>`、`assistant`、`\n`）均設為 `label = -100`；（ii）每輪的第一個 answer token 均為受監督；（iii）每輪助理回覆末尾的 `<|im_end|>` 均為受監督，使模型學習何時停止生成；（iv）全部助理輪次均被正確識別與處理。

![圖 SFT-V 邊界細節：多輪對話邊界驗證與 SFT 標籤對齊](./assets/plots/sft_mask_verification.png)

#### 7.2.8 統一對話語法與推論前綴

所有異質資料來源都被嚴格映射到同一 ChatML 對話語法。對於單輪問答，標準化後的模板為：
...
多輪對話則以連續的 user-assistant 區塊串接，模型在每個助理位置都能透過 Causal Attention 回看完整對話歷史，但 loss mask 只在助理答案與終止符上啟動。下表列出 SFT 訓練池中的四段代表性助理輸出（來自 `mix_a25_u75_cot_hf`），均已嚴格對齊至同一 ChatML 語法：
...
在**推論（Inference）**時，我們只餵給模型開放式的 Assistant 前綴，讓它接續生成：
...
此時終止 token `<|im_end|>`（id 32001）尚未出現；模型必須自行生成完整回覆後預測終止符，才能結束當輪輸出。這一「訓練時監督終止符、推論時預測終止符」的設計閉環，是 ChatML 格式相較 LLaMA-style 格式的核心優勢之一，避免了無限延伸與跨語言亂碼等常見問題（見 §7.2）。

![圖 C3：ChatML 對話模板與推論前綴可視化](./assets/plots/chatml_template.png)
...
![Mamba AI Assistant — App 介面預覽](./assets/uiux/iphone_preview.png)

_圖 9a：Mamba AI 助手 iOS 原型介面，呈現待機、處理中、回應三種操作狀態。_
...
![端到端系統資料流圖](./assets/uiux/audio_flow.png)

_圖 9b：端到端系統資料流。使用者語音輸入經 Apple Neural Engine 前處理後，由 MLX 後端驅動的 Hybrid Mamba-TuckerMoE 模型進行推論，輸出分兩路：語音合成（TTS）與 UI 狀態更新。_

![實體情境模擬圖（一）](./assets/uiux/user_story1.png)

_圖 9c：使用情境模擬（一）。iPhone 16 Pro 搭配 MagSafe 旋轉支架，轉化為桌面免持 AI 助手，呈現日常辦公場景下的即時語音問答互動。_

![實體情境模擬圖（二）](./assets/uiux/user_story2.png)

_圖 9d：使用情境模擬（二）。展示不同場景下的裝置擺放與互動方式，說明本應用在有限硬體資源下支援全天候語音查詢的可行性。_
