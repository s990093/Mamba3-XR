...
def configure_cuda_runtime():
    ...
    if major >= 8:
        ...
        print(f"🚀 高階 GPU ({gpu_name}, sm_{major}{minor}): bf16 + TF32 + FlashSDP + cuDNN benchmark")
        return "bf16"
    ...
    print(f"🐢 非高階 GPU ({gpu_name}, sm_{major}{minor}): FlashSDP 關閉，改用 fp16 + fallback SDPA")
    return "fp16"
...
if __name__ == "__main__":
    ...
    GRADIENT_ACCUMULATION_STEPS = 32  # Effective batch = BATCH * ACCUM * n_gpu
    LR = 8e-5

    WARMUP = 400   # 4%
    STEPS                    = 120000 # 總訓練 steps
    CHECKPOINT_EVERY         = 500 #eps 存一次
