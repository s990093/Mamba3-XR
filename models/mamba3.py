...
class Mamba3Block(nn.Module):
    ...
    def chunk_parallel_scan(self, u, dt, A, C, chunk_size=128):
        ...
        # decay_intra: (B, C, L, H)
        # Optimized: Replace einsum with broadcast multiply
        h_effect = h_states_inter.unsqueeze(2) * decay_intra.unsqueeze(-1).unsqueeze(-1)  # (B, C, L, H, N, P)
        
        # y_off = C^T * h_effect
        # Optimized: Replace einsum with matmul (reuse c_for_mat from y_diag)
        h_eff_trans = h_effect.permute(0, 1, 2, 3, 5, 4).reshape(batch_dims, P, N)
        y_off_flat = torch.matmul(h_eff_trans, c_for_mat)  # (batch, P, R)
