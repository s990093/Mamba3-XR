...
def main():
    ...
    # FP16 config -> Kaggle T4 Amp initialization
    ...
    if os.path.exists(checkpoint_path):
        ...
        raw_ckpt = ckpt['model']
        has_module = list(raw_ckpt.keys())[0].startswith("module.")
        if num_gpus > 1 and not has_module:
            raw_ckpt = {"module."+k: v for k, v in raw_ckpt.items()}
        elif num_gpus <= 1 and has_module:
            raw_ckpt = {k.replace("module.", ""): v for k, v in raw_ckpt.items()}
        model.load_state_dict(raw_ckpt)
        optimizer.load_state_dict(ckpt['optimizer'])
        scheduler.load_state_dict(ckpt['scheduler'])
        start_step = ckpt['step']
        print(f"✅ Resumed from Step {start_step} / {ckpt.get('total_steps', '?')} | " 
    ...
    print(f"🚀 開始訓練！Global Batch Size = {BATCH_SIZE * GRADIENT_ACCUMULATION_STEPS * max(1, num_gpus)} "
          f"({BATCH_SIZE}/GPU × {GRADIENT_ACCUMULATION_STEPS} accum × {max(1, num_gpus)} GPUs)")
    ...
    while global_step < STEPS:
        ...
        with torch.amp.autocast('cuda'):
            # 訓練時只需要 loss，避免 DataParallel 將整個 logits 計算圖 gather 回單卡
            loss, raw_aux = model(x, labels=y)
            if num_gpus > 1:
                loss = loss.mean()
                raw_aux = raw_aux.mean()
            loss_for_log = loss.item()  # capture before scaling
            aux_for_log = raw_aux.item()
        ...
        if batch_idx % GRADIENT_ACCUMULATION_STEPS == 0:
            ...
            # AMP 安全步進：只在 optimizer.step() 沒被跳過時才推進 scheduler
            ...
            # GPU memory usage (in GB) on each device
            if torch.cuda.is_available():
                mem_gb_0 = torch.cuda.memory_allocated(0) / 1e9
                mem_gb_1 = torch.cuda.memory_allocated(1) / 1e9 if num_gpus > 1 else 0.0
            else:
                mem_gb_0 = 0.0
                mem_gb_1 = 0.0
            ...
            log_line = (
                ...
                f"Mem[GB] GPU0={mem_gb_0:.2f}, GPU1={mem_gb_1:.2f}"
            )
            print(f"✅ {log_line}")
